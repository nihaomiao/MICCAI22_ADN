...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, scale_factor=(2, 2, 2), basic_module=DoubleConv,
                 conv_layer_order='gcr', num_groups=8, mode='nearest', padding=1, upsample=True, use_transconv=True):
        ...
        if upsample:
            if basic_module == DoubleConv:
                ...
                self.upsampling = UpsamplingConv(in_channels=in_channels, out_channels=out_channels)
                # sum joining
                self.joining = partial(self._joining, concat=False)
                # adapt the number of in_channels for the ExtResNetBlock
                in_channels = out_channels
...
def create_decoders(f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups, upsample, use_transconv):
...
class UpsamplingConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv3d = torch.nn.Conv3d(in_channels=in_channels, out_channels=out_channels, kernel_size=(3, 3, 3),
                                      padding=(1, 1, 1))
